...
class SDXLMergeKarcher:
    ...
    def resize_tensors(self, tensors):
        """Aligns tensors by padding with zeros if the last two dimensions differ."""
    ...
    def extract_tensor_from_patch(self, patch):
        """Extracts the tensor from a patch, handling tensors or tuples."""
    ...
    def preserve_patch_format(self, original_patch, merged_tensor):
        """Preserves the original patch format (tensor or tuple) with the merged tensor."""
    ...
    def merge(self, num_models, model_clip_weight1, model_clip_weight2,
        ...
        # Extract models, CLIP, and weights
        ...
        for key in model_keys:
            ...
            # Get list of patches for the key
            ...
            # Extract the first valid tensor to determine dtype and device
            try:
    ...
    def karcher_merge_tensors(self, tensors, alphas, max_iter, tol):
        """ИСПРАВЛЕННАЯ реализация Karcher mean согласно алгоритму из статьи."""
        if len(tensors) == 1:
            return tensors[0]

        # Декомпозиция на нормы и направления
        norms = []
        units = []
        ...
        # Инициализация среднего направления
        ...
        # Итерационный алгоритм Karcher mean
        for _ in range(max_iter):
            T = torch.zeros_like(u, dtype=torch.float)
            for a, ui in zip(normalized_alphas, valid_units):
                dot = torch.clamp(torch.dot(u.flatten(), ui.flatten()), -1.0, 1.0)
                theta = torch.arccos(dot)  # ✅ ИСПРАВЛЕНО: используем arccos
                theta_val = theta.item()
                if theta_val < tol:
                    continue
                sin_theta = torch.sin(theta)  # ✅ ИСПРАВЛЕНО: используем sin_theta
                T += a * (theta / sin_theta) * (ui - dot * u)  # ✅ ИСПРАВЛЕНО: делим на sin_theta
